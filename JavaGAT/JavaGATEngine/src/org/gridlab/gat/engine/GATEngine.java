...
public class GATEngine {
     ...
     */
    public AdaptorList getAdaptorList(Class cpiClass) throws GATObjectCreationException {
        if (!adaptors.containsKey(cpiClass.getName())) {
            ...
            throw new GATObjectCreationException("getAdaptorList: No adaptors loaded for type "
                    + cpiClass.getName());
        } else
            return (AdaptorList) adaptors.get(cpiClass.getName());
    }
	...
	protected void readJarFiles() {
		...
		for(int i=0; i<adaptorPathList.size(); i++) {
		    ...
		    try {
		        URL u = new URL("file://" + jarFile.getName());
	...
	protected void loadCpiClass(JarFile jarFile, Manifest manifest, Attributes attributes,
	...
	protected void loadCPIClassesFromJar(JarFile jarFile) {
		...
		loadCpiClass(jarFile, manifest, attributes, "Endpoint", EndpointCpi.class);
		...
		loadCpiClass(jarFile, manifest, attributes, "LogicalFile", LogicalFileCpi.class);
	 ...
	 */ 
	public Advertisable unmarshalAdvertisable(GATContext gatContext, Preferences preferences, String input)
			throws GATInvocationException {
		if (input == null) {
		...
		for (int i = 0; i < unmarshallers.size(); i++) {
			...
			try {
				Method m = c.getMethod("unmarshal",
						new Class[] { GATContext.class, Preferences.class, String.class });
				Advertisable res = (Advertisable) m.invoke(null,
						new Object[] { gatContext, preferences, input });
				if (res != null) {
					if(DEBUG) {
						System.err.println("unmarshalAdvert: returning: " + res);
					}
					// success!
					return res;
				}
				...
				if(DEBUG) {
					System.err.println("unmarshaller for " + c.getName() +" failed:" + e);
				}
	...
	public static void registerAdvertisable(Class unmarshaller) {
		...
		try {
			...
			// {Advertisable.class});
			Method m2 = unmarshaller.getMethod("unmarshal",
					new Class[] { GATContext.class, Preferences.class, String.class });
		} catch (Exception e) {
	...
	public static Advertisable defaultUnmarshal(Class type, String s) {
		...
		try {
			if(DEBUG) {
				System.err.println("default unmarshaller start, type = " + type + " string = " + s);
			}
			...
			Advertisable res = (Advertisable) unmarshaller
					.unmarshal(sr);
			if(DEBUG) {
